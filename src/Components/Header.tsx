...
const Header = (props: pageProps) => {
  ...
  const handleInternalClick = (cb?: () => void) => () => {
                    ...
                    >
                      {item.external ? (
                        ...
                        <Link
                          to={item.href}
                          onClick={handleInternalClick()}
                          className="text-indigo-600 hover:text-indigo-700 font-bold text-lg transition-colors duration-200 relative group"
                    ...
                    {navItems.map((item) => (
                      <li key={item.name} className="w-full text-center">
                        {item.external ? (
                          ...
                          <Link
                            to={item.href}
                            onClick={handleInternalClick(toggleMenu)}
                            className="text-2xl font-medium text-gray-800 hover:text-indigo-600 transition-colors duration-200 block py-3"
                            aria-label={item.name}
                          >
                            {item.name}
